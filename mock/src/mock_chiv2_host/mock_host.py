...
def heartbeat(address: str, unique_id: str, key: str):
    response = requests.post(address+f"/api/v1/servers/{unique_id}/heartbeat", headers={"x-chiv2-server-browser-key": key})
    if not response.ok:
...
def main():
    ...
    for server in servers:
        ...
        print("Registered server on port " + str(server["port"]) + " with unique id " + result["server"]["unique_id"])

        registered_servers.append({"key": result["key"], "unique_id": result["server"]["unique_id"]})
