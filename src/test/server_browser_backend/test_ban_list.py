...
def test_load_ban_list_file():
    key = "foo"
    ban_list = BanList(key, "src/test/resources/test_load_ban_list.txt")
...
def test_update_ban_list_file():
    clean_up_file("src/test/resources/test_update_ban_list.txt")
    key = "foo"
    ban_list = BanList(key, "src/test/resources/test_update_ban_list.txt")
    ...
    with open("src/test/resources/test_update_ban_list.txt", "r") as f:
        lines = f.readlines()
        assert len(lines) == 1
...
def test_add_all_to_ban_list():
    clean_up_file("src/test/resources/test_add_all_to_ban_list.txt")
    key = "foo"
    ban_list = BanList(key, "src/test/resources/test_add_all_to_ban_list.txt")
    assert len(ban_list.get_all()) == 0
